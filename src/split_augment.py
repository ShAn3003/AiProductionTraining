...
# 数据增强操作
def augment_image_and_labels(image, label_path):
    augmented_images = [image]  # 保留原始图像
    augmented_labels = [label_path]
...
for img_file in image_files:
    ...
    txt_path = os.path.join(labels_dir, img_file.replace('.png', '.txt'))
    ...
    if lines:
        ...
        augmented_imgs, augmented_lbls = augment_image_and_labels(image, txt_path)
        for i, (aug_img, aug_lbl) in enumerate(zip(augmented_imgs, augmented_lbls)):
            aug_img_file = f"{os.path.splitext(img_file)[0]}_aug_{i}.png"
            aug_img_path = os.path.join(images_dir, aug_img_file)
            aug_lbl_path = os.path.join(labels_dir, aug_img_file.replace('.png', '.txt'))
...
keep_no_target = int(len(augmented_images) * 0.1)  # 保留10%的无目标图像
...
for i, img_file in enumerate(augmented_images):
    ...
    src_txt_path = src_img_path.replace('.png', '.txt')
    ...
    shutil.copy(src_txt_path, os.path.join(dst_dir, "labels", img_file.replace('.png', '.txt')))
